...
while True:
    ...
    context= torch.tensor(encode(prompt), dtype=torch.long,device=device) #context is a tensor of zeros of size (1,block_size)
    generated_chars= decode(m.generate(context.unsqueeze(0), max_new_tokens=150)[0].tolist()) #generate 150 new tokens
    print(f'Completion : \n {generated_chars}')